...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_SHIPMENT: 'shipment/PARTIAL_UPDATE_SHIPMENT',
...
export default (state: ShipmentState = initialState, action): ShipmentState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_SHIPMENT):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_SHIPMENT):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_SHIPMENT):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IShipment> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_SHIPMENT,
