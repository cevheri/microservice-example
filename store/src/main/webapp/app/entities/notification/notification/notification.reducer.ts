...
export default (state: NotificationState = initialState, action): NotificationState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_NOTIFICATION):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_NOTIFICATION):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_NOTIFICATION):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<INotification> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_NOTIFICATION,
